...
func (r rawBytes) MarshalMsg(b []byte) ([]byte, error) {
	return hsp.AppendBytes(b, []byte(r)), nil
}

func (r rawBytes) Msgsize() int {
	return hsp.BytesPrefixSize + len(r)
}

func (r *rawBytes) UnmarshalMsg(b []byte) ([]byte, error) {
	tmp, out, err := hsp.ReadBytesBytes(b, (*(*[]byte)(r))[:0])
	*r = rawBytes(tmp)
	return out, err
}
...
func TestReadWriteFile(t *testing.T) {
	...
	err = hsp.WriteFile(rawBytes(data), f)
	if err != nil {
		t.Fatal(err)
	}
...
func BenchmarkWriteReadFile(b *testing.B) {
	...
	b.ResetTimer()
	err = hsp.WriteFile(data, f)
	if err != nil {
		b.Fatal(err)
	}
	err = hsp.ReadFile(&data, f)
	if err != nil {
		b.Fatal(err)
	}
}
